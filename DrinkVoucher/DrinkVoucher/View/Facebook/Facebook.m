...
-(void)setFcaebookWithDelegate:(id<FBSessionDelegate>)delegate
{
    NSLog(@"self.fbappId : %@",self.fbappId);
    
    [self initWithAppId:self.fbappId urlSchemeSuffix:nil andDelegate:delegate];
}
...
- (void)dialog:(NSString *)action
   ...
   andDelegate:(id <FBDialogDelegate>)delegate {
    ...
    if ([action isEqualToString:kLogin]) {
        ...
        [params setObject:fbappId forKey:@"app_id"];
        if ([self isSessionValid]) {
